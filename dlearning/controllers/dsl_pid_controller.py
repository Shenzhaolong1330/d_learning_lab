...
class DSLPIDController(nn.Module):
    def __init__(self, dt: float, g: float, uav_params) -> None:
        ...
        # self.I_COEFF_FOR = nn.Parameter(torch.tensor([0.5, 0.5, 1.5]))
        ...
        self.I_COEFF_FOR = nn.Parameter(torch.tensor([0.0, 0.0, 0.02]))
        ...
        self.G = nn.Parameter(torch.tensor([0, 0, abs(g)]))
    ...
    def forward(
        ...
        if control_target is None:
            control_target = torch.zeros_like(state[..., :7])

        # parsing input
        pos, quat, vel, angvel = torch.split(state[..., :13], [3, 4, 3, 3], dim=-1)
        ...
        integral_pos_error = torch.clip(integral_pos_error + pos_error * self.dt, -2.0, 2.0)
        integral_pos_error[...,2] = torch.clip(integral_pos_error[...,2], -0.15, 0.15)
